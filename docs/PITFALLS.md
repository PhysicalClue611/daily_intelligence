...
### 81. 跨源印证指纹复用"打分用宽松关键词表"导致假阳性（issue #19 follow-up，PR #46 review）

（2026-07-18 发现修复，PR #46）修复 issue #19 的单token/全大写实体漏检（`_PHRASE_RE` 抓不住"Hormuz"/"US"）时，直接复用了 `score_and_filter()` 已有的关键词锚定表（`anomaly_tickers` + `geo_keywords` 拆分出的多词短语单词，如"Middle East"→"middle"/"east"）作为跨源印证指纹的补充匹配集。上线前用真实数据复现验证：两篇完全无关的报道（"East Coast 停电" vs "East Asia 芯片需求"）仅因共享拆分词"east"就被判定为"交叉印证"；两篇同 ticker 但完全不同事件的报道（财报 vs CEO 离职）仅因共享 ticker 本身也被误判为已印证。

**根因**：`score_and_filter()` 的关键词表是为"打分加分项"设计的——单个词命中只贡献 +0.05 排序权重，误报代价低；但 `_compute_corroboration()` 把同一张表当"独立信源确认了同一事实"的判定依据，误报代价高得多（会让 Pass 2 把弱关联断言当既成事实陈述）。两个用途对关键词表的精确度要求不同，共用同一份表本身就是设计缺口。

**发现路径**：本次修复由 Grok CLI 4.5 对 PR #46 的 pending inline review 指出（该 review 逐一验证 `_PHRASE_RE`/真实 archive 数据/watchlist.md 配置后给出，未经验证前先复现确认属实），这个精度回归是本人自己在同一 PR 里引入的，且在低效力和中效力两轮 `/code-review` 自查中均未被发现——纯规则/纯正则改动的副作用比表面看起来更容易被自己的 finder agent 漏过，交叉验证（哪怕是另一个 LLM 的 review）仍有实际价值。
...
### 89. `call_llm()` 新增免 JSON 文本路径时沿用旧的 `content or reasoning_content or reasoning` 取值顺序，`finish_reason=length` 时会把部分思维链错误当正文返回（issue #60/PR #62 review）
（2026-08-05 发现修复，`blacktomb42` code review）issue #60 给 `llm_client.py::call_llm()` 加了 `parse_json=False` 模式（`report_pass2` 的 `report_md` 改为直接输出裸 markdown，不再套 JSON 壳），实现时直接照搬了原有 JSON 路径的取内容逻辑：`content = msg.get("content") or msg.get("reasoning_content") or msg.get("reasoning") or ""`。这行代码本身是 PR #56 之前就有的旧写法——`finish_reason=="length"` 且 `content` 为空（reasoning 吃满预算，issue #53/#59/#60 反复出现的故障形态）时，`reasoning_content` 里的部分思维链会被当成"内容"取出来。在旧的 JSON 路径下这个问题被"意外"掩盖：思维链文本通常解析不出合法 JSON，`parse_llm_json` 会失败，走既有的重试/fallback 逻辑，表面上看起来没问题。但 `parse_json=False` 路径没有 JSON 解析这道意外的安全网——思维链文本本身就是"合法的纯文本"，会直接被当作 `result["text"]` 返回、标记为成功，对 `report_pass2` 而言就是把部分思维链当成当日报告正文发邮件/发TG/写Obsidian，且不会触发重试或 fallback_model。这正是 PR #56 在 `telegram_commands.py::_parse_step4_response()` 修过的同一类 bug（"CoT 被错误提升为答案"）在新调用路径里的重现，只是触发条件从"Telegram Step4 的 thinking 配置"换成了"任何调用 `parse_json=False` 的 stage"。**修复**：新增 `_resolve_content(msg, finish_reason)`，镜像 `_parse_step4_response` 的判断顺序——`content` 非空直接用；`content` 为空且 `finish_reason=="length"` 则返回空字符串（绝不提升 `reasoning_content`）；`content` 为空但 `finish_reason` 不是 `length`（如某些 provider 把最终答案放在 `reasoning_content` 但没有截断）才允许 fallback 到 `reasoning_content`/`reasoning`。primary 请求和 OR-flex-fallback 请求两条路径都要改，只改一处会漏。**教训**：修复过的 bug 模式如果只在触发它的那一个函数里改掉症状（`_parse_step4_response`），根因（"CoT 取值优先级本身允许在 length 场景下泄漏"）留在别处未变的代码里，下次新增一条相似调用路径时会原样复制过去——多个模块间存在同源逻辑（这里是"取 LLM 响应内容"）时，修 bug 应该问一句"这段逻辑在别处是否有平行副本"，不能只满足于当前这一个函数不再报错。

### 95. `call_llm()` 免 JSON 路径只拒绝空正文、接受被 length 截断的非空正文；推理模型提档而 `max_tokens` 不跟着加，残缺报告被当成功发出（PR #94）

（2026-09-23 PM 发现，PR #94 修复）issue #90/PR #91 把 `report_pass2` 切到 `gpt-6-luna` + `reasoning.effort=xhigh`，`max_tokens` 仍是 16000。推理和正文共用这份预算：`prompt=16792 completion=16000 reasoning=15781 finish_reason=length`，正文只剩约 219 token，报告在第一节“INTC（持仓）收于$122.57，”处断掉。邮件、TG、Obsidian 照常写出，运行状态显示 Pass 2 成功，没有告警。

**根因**：上一条（坑 89）修的是“空正文 + length 时不把思维链提升为正文”，`_resolve_content()` 因此只在正文为空时把 length 当失败；非空但被截断的正文照样作为成功返回，不重试、不 fallback。PR #62 当时还有测试 `test_call_llm_parse_json_false_accepts_partial_content_on_length` 专门锁住这个行为——当时的判断是“截断在纯文本里只丢尾巴，比 JSON 整份作废好”，没想到对日报而言“残缺报告当完整报告发出”本身就是静默失败。另一个触发条件是模型提档：推理强度从 high 升到 xhigh，推理量约翻倍，旧预算不再够用，而配置切换 PR 里没人重算预算。

**修复**：`parse_json=False` 路径新增 `_free_text()`，`finish_reason=length` 即使正文非空也抛可重试错误，依次走同模型重试 → `fallback_model` → `run_finance` 输出代码生成的情报快照摘要并发 TG 告警。`report_pass2.max_tokens` 16000→32000（`llm_config.py` DEFAULTS、`llm_config.json`、`llm_config.example.json` 三处）。PR #62 的测试改为相反的预期。

**连带**：`max_tokens` 翻倍时 HTTP 超时也要跟着放宽。`call_llm()` 非流式请求原先固定 180s 读超时，32000 token 的 xhigh 调用会超时。改为 `_http_timeout()` = `max(180, max_tokens // 50)`，32000 对应 640s。补跑实测 Pass 2 约 143s（completion 12336，reasoning 10358）。

**教训**：① 对产出直接交付给用户的 stage，`finish_reason=length` 一律是失败，不论正文空不空；② 切推理模型或推理强度时，`max_tokens` 和 HTTP 超时要在同一个 PR 里重算，并用一次真实调用看 reasoning 占比，而不是沿用旧值。
