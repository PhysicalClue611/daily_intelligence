...
## 当前系统状态（2026-08-03/04，issue #58 / issue #59，纯排查，无代码改动）
...
9. **issue #59：`report_pass1` 模型重新选型**（2026-08-03/04 起）：用 `report_pass1` 真实 prompt 验证 `google/gemma-4-31b-it` 或 `openai/gpt-5.6-luna`（非pro）能否替代 `deepseek/deepseek-v4-flash`，同时评估 `evaluate_am_calibration()` 是否该迁移到不同的 stage 配置；验证前先看 issue #59 完整证据链，不要重复已经测过并证伪的"切换到 `-latest`"或"只加大 `max_tokens`"两条路径
